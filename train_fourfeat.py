...
fourfeat_model = networks.MLP(in_features=1, out_features=1, n_neurons=256, n_layers=4,
                              embedding_size=embedding_size, scale=scale, hidden_act=nn.SiLU(), output_act=nn.SiLU())
model = fourfeat_model
...
with torch.no_grad():
    ...
    title = 'fourfeat_s'+str(scale)+'_'+str(step)
...
# utils.save_densities(density, grid_dimensions, title, save_density, True, path='logs/densities/fourfeat_multires/')
