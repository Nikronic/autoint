...
mlp_model = networks.MLP(in_features=1, out_features=1, n_neurons=512, n_layers=6, scale=0,
                        embedding_size=0, hidden_act=nn.SiLU(), output_act=None)  # learning x^2
model = mlp_model
...
# test model
integral_pred = model(x)
sys.stderr.write('Grad network allclose: {}\n'.format(torch.allclose(input=utils.gradient(integral_pred, x), other=f(x),
